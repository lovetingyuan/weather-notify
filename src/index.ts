	...
	try {
		...
		for (let i = 0; i < weatherCodes.length; i++) {
			...
			if (info.isBad) {
				...
				occurrences.push(
					`🕒 ${timeStr} | 🌡️ ${temps[i]}°C | ${info.emoji} ${info.desc}`,
				);
...
function getWeatherInfo(code: number): {
	...
	if (code >= 1 && code <= 3)
		return { desc: "多云", emoji: "⛅", isBad: false };
	if (code >= 45 && code <= 48)
		return { desc: "雾天", emoji: "🌫️", isBad: false };
	if (code >= 51 && code <= 57)
		return { desc: "毛毛雨", emoji: "🌧️", isBad: true };
	if (code >= 61 && code <= 67)
		return { desc: "雨天", emoji: "🌧️", isBad: true };
	if (code >= 71 && code <= 77)
		return { desc: "雪天", emoji: "❄️", isBad: true };
	if (code >= 80 && code <= 82)
		return { desc: "阵雨", emoji: "🌦️", isBad: true };
	if (code >= 85 && code <= 86)
		return { desc: "阵雪", emoji: "🌨️", isBad: true };
	if (code >= 95 && code <= 99)
		return { desc: "雷暴", emoji: "⛈️", isBad: true };
