    ...
    if (url.pathname === "/api/locations") {
      if (req.method === "GET") {
        ...
        try {
          ...
          if (existingIndex >= 0) {
            locations[existingIndex] = { name: body.name, lat: body.lat, lon: body.lon };
          } else {
      ...
      try {
        ...
        if (locString) {
          ...
          for (const loc of locations) {
            await checkWeatherAndNotify(body.user, loc);
          }
        }
      ...
      for (const key of result.keys) {
        ...
        fetchPromises.push(
          (async () => {
            ...
            for (const loc of locations) {
              await checkWeatherAndNotify(username, loc);
            }
          })(),
        );
      }
...
async function checkWeatherAndNotify(username: string, loc: Location) {
  try {
    ...
    for (let i = 0; i < weatherCodes.length; i++) {
      ...
      if (info.isBad) {
        ...
        const timeStr = hourlyTimes[i].substring(5).replace("T", " ");
...
function getWeatherInfo(code: number): { desc: string; emoji: string; isBad: boolean } {
